...
const personalConn = mongoose.createConnection(
  "mongodb+srv://Aditya3236:Poi%4009876@cluster0.ot7jbiv.mongodb.net/Personal"
);
const experienceConn = mongoose.createConnection(
  "mongodb+srv://Aditya3236:Poi%4009876@cluster0.ot7jbiv.mongodb.net/Experience"
);
const educationConn = mongoose.createConnection(
  "mongodb+srv://Aditya3236:Poi%4009876@cluster0.ot7jbiv.mongodb.net/Education"
);
...
const EducationCertificate = educationConn.model(
  "EducationCertificate",
  EducationCertificateSchema
);
const ExperienceCertificate = experienceConn.model(
...
const verifyDocument = async (documentIdentifier, document) => {
  ...
  switch (documentIdentifier) {
    case "PAN": {
      ...
      let { name, pan_number, father_name } = document;
      ...
      if (
        pan.name === name.trim() &&
        pan.pan_number == pan_number.trim() &&
        pan.father_name == father_name.trim()
      ) {
        verified = true;
        break;
      }
    ...
    case "VOTERID": {
      const { name, passport_number, father_name, elector_name, dob } =
        document;
    ...
    case "CBSE": {
      ...
      const educationCertificate = await EducationCertificate.findOne({
        roll_number,
      });
      if (
        educationCertificate &&
        ...
        educationCertificate.admission_year.getTime ===
          new Date(admission_year).getTime &&
        educationCertificate.passing_year.getTime ===
          new Date(passing_year).getTime &&
        ...
        marks_list.every(
          (m, i) =>
            m.subject === educationCertificate.marks_list[i].subject &&
            m.marks === educationCertificate.marks_list[i].marks
        )
      ) {
        verified = true;
      }
    ...
    case "EXPERIENCE": {
      ...
      const experienceCertificate = await ExperienceCertificate.findOne({
        employee_id,
      });
      if (
