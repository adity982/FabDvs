...
app.post("/uploadDocument", async (req, res) => {
  ...
  if (isDocVerified) {
    ...
    const documentToBeVerified = {
      ...
      data: encryptData(
        document.data,
        companyToSmallOrgMap[documentIdentifier]
      ),
      requestBy: orgName,
      status: "",
    };
    ...
    res.json({
      message: "Document is uploaded to be verified",
...
const MONGOURL ="mongodb+srv://Aditya3236:Poi%4009876@cluster0.ot7jbiv.mongodb.net/Personal";
...
httpServer.listen(port, async () => {
  ...
  try {
    ...
    //   console.log(`Block: %S`, blockString);
    // });
...
const contractListener = async (event) => {
  ...
  if (
    documentPayload.document.requestBy === orgName &&
    documentPayload.status === VERIFIED
  ) {
    io.emit("log", {
      time: new Date(),
      id: document.id,
    ...
    console.log(
      "\x1b[4m\x1b[47m\x1b[34m%s\x1b[0m",
      "-------- Received Verified Document -------\n"
    );
    console.log(
      "\x1b[1m\x1b[34m%s\x1b[0m",
      `${JSON.stringify(documentPayload, null, 2)}`
...
const decryptData = (encryptedData, identifier) => {
  ...
  const decryptData = crypto.privateDecrypt(
    {
      //this is in buffer - so first convert to string then to buffer
      key: privateKey,
      padding: crypto.constants.RSA_PKCS1_OAEP_PADDING,
      oaepHash: "sha256",
    },
